...
SCALE_FACTOR = 10.0 # Josh used 6.0
## 9.0 seems too low and 12.0 too high for TC
...
class BoundaryButtons():
    ...
    def createButtons(self):
        for i in range(50):
            ...
            button.setGeometry(-100,20+i*50,60,20)
    ...
    def updateBoundaries(self, structure_ids, border_locs):
        ...
        for i, border in enumerate(borders):
            try:
                name = structure_tree[structure_tree.id == structure_ids[border]]['acronym'].iloc[0]
            except IndexError:
                name = 'none'
...
class App(QWidget):
    ...
    def __init__(self):
        super().__init__()
        self.title = 'Tissuecyte Boundary Refinement'
        self.left = 300
        self.top = 100
        self.width = 1600
        self.height = 800
    ...
    def initUI(self):
        ...
        self.current_directory = '//allen/programs/braintv/workgroups/tiny-blue-dot/zap-n-zip'
    ...
    def clickedOnImage(self , image, event):
        ...
        if x < 100 and image.objectName() == self.selected_probe:
            self.anchor_points[self.selected_boundary, INDEX_MAP[self.selected_probe]] = int(y / (SCALE_FACTOR / 3))
                ...
                if (probe == probe_to_refresh or probe_to_refresh == None):
                    ...
                    if len(structure_ids) > 0:
                        ...
                        if imQt.height() == 2400:
                            ...
                            self.boundary_buttons[i].updateBoundaries(structure_ids, border_locs * (SCALE_FACTOR / 3)) # Josh says *2
                            channels = 384 - (border_locs * SCALE_FACTOR / 2400 * 384)

                            self.df.loc[self.df.probe == probe, 'channels'] = np.round(channels).astype(int)
    ...
    def loadData(self):
        ...
        if fname.split('.')[-1] == 'csv':
            expnum = fname[fname.find('.csv')-1:fname.find('.csv')]
            self.output_file = os.path.join(self.current_directory, 'final_ccf_coordinates_exp{}.csv'.format(expnum))
            self.anchor_points_file = os.path.join(self.current_directory, 'coordinate_anchor_points_exp{}.npy'.format(expnum))

            self.setWindowTitle(os.path.dirname(fname))
            self.df = pd.read_csv(fname)
            self.df['channels'] = 0

            self.images = [QImage(self.current_directory + '/physiology_' + i + '_exp' + expnum + '.png') for i in self.probes]
